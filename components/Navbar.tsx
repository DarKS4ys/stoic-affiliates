...
export default function Navbar() {
  ...
  useMotionValueEvent(scrollY, 'change', (latest) => {
    ...
    if (previous && latest > previous && latest > 49) {
      setHidden(true);
    } else {
      setHidden(false);
    }
  ...
  return (
    <motion.nav
      ...
      className={
        'z-50 sticky top-0 w-full px-8 py-6 text-white backdrop-blur-lg bg-gradient-to-b from-background via-background/70 to-transparent'
      }
    >
      <div className="w-full flex items-center justify-between max-w-6xl mx-auto">
        <div className="flex items-center gap-3 w-fit">
          <h1 className="md:block hidden font-medium text-[1.24rem]">STOIC</h1>
          <Image
            alt="Stoic Logo"
            src={Logo}
            width={15}
        ...
        <div className="flex gap-5 justify-end items-center">
          <Link
            ...
            target="_blank"
            href='/sign-in'
            className="active:scale-90 bg-background hover:bg-border/50 transition duration-200 border border-border px-4 py-2 font-semibold text-white flex items-center gap-2"
          >
            <p>Login</p>
            <FaUserCircle />
          </Link>
